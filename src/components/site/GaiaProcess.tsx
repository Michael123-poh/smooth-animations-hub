...
export function GaiaProcess() {
  ...
  useEffect(() => {
    function tick() {
      ...
      // ── Step tracking (bidirectionnel) ───────────────
      // progress = 0 when top of section is at bottom of viewport
      //          = 1 when bottom of section is at top of viewport
      const scrolled = vh - rect.top;          // px scrolled into section
      const total    = rect.height + vh;       // total scroll travel
      const pct      = Math.max(0, Math.min(1, scrolled / total));

      // Map 0..1 into 4 equal buckets → step 0..3
      const step = Math.min(steps.length - 1, Math.floor(pct * steps.length));
      setActiveStep(step);
  ...
  return (
    <section
      className="gaia-process"
      id="processus"
      aria-labelledby="process-heading"
      ref={sectionRef}
    >
      {/* Pattern overlay parallax */}
      <div
        ref={patternRef}
        aria-hidden="true"
        style={{
                  ...
                  <div className="step-content">
                    ...
                    <div
                      style={{
                        ...
                        overflow: "hidden",
                        transition:
                          "opacity 0.4s, max-height 0.4s cubic-bezier(0.4,0,0.2,1), margin-top 0.4s",
                      }}
                    >
          ...
          <div className="process-visual">
            <div className="process-visual-card">
              ...
              {steps.map((s, i) => {
                ...
                const barW    = done ? "100%" : current ? "60%" : "0%";
